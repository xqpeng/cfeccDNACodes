feature_CNV <- function(bamfile){
  ...
  # load oncogene list ----
  data("Immugene", package = "EccDNAFeature")
...
reads_gr <- granges(readGAlignments(bamfile))
  ...
  cat("\nprocessing ", nrow(Immugene), " immun genes...\n")
  
  # process each Immugene ----
  for (i in 1:nrow(Immugene)) {
    
    current_gene_info <- Immugene[i, ]
    ...
    if (is.na(gene_length) || gene_length <= 0) {
      warning(paste("Immugene '", gene_name, "' has invalid length, skip.", sep = ""))
      next
    }
    ...
    if (nrow(model_data) > 100) {
      ...
      warning("Immugenee '", gene_name, "' (in BAM '", basename(bamfile), "') does not have sufficient data, and it cann't build reliable GC model. Skip GC correction!")
      gene_gc_content <- NA 
    }
  ...
  cat("---------------------------------------------------\n")
  cat("CNV_immu profile of  '", basename(bamfile), "'  is finished！\n\n")
