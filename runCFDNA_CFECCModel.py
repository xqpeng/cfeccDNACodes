...
def loadFeature(Case_path, Control_path, X_train, X_test, Feature):
    ...
    def clean_filename(fname, feature_name):
        ...
        if feature_name in ['BPM_CFDNA', 'EDM_CFDNA']:
            feature_name = feature_name.replace(f"_CFDNA", "")                
    ...
    def read_and_process(file_path):
        try:  
            if Feature in ['BPM', 'EDM', 'SBM', 'JNM', 'OJM', 'BPM_CFDNA', 'EDM_CFDNA']:
                ...
                tmp_data = pd.read_csv(file_path, sep='\t', header=0, names=['1', '2', '3', '4', 'log'])
                tmp_data['value'] = tmp_data['log'].replace(np.nan, 0)            
                return tmp_data['value'].to_list()
            elif Feature in ['FSC']:
                tmp_data = pd.read_csv(file_path, sep='\t', header=0, names=['region', 'short', 'median', 'long', 'total'])
                short = tmp_data['short'].to_list()
                median = tmp_data['median'].to_list()
                long = tmp_data['long'].to_list()
                f = short + median + long
                f = np.nan_to_num(f)
                return f
            elif Feature in ['FSD']:
                return tmp_data['value'].tolist()
                tmp_data = pd.read_csv(file_path, sep='\t', header=None)
                melted_data = pd.melt(tmp_data, id_vars=[0], var_name='Column2', value_name='Value')
                a = melted_data['Value'].to_list()
                a = np.nan_to_num(a)
                return a 
            else:
                return tmp_data['value'].tolist()                        
...
def main():
    ...
    feature_type = ['BPM', 'EDM', 'SBM', 'JNM', 'OJM', 'CNV_onco', 'CNV_im', 'OLR', 'BPM_CFDNA', 'EDM_CFDNA', 'CNV', 'FSC', 'FSD']
    ...
    print("\nTraining Meta-Model (CFDNA+CFECC)...")
    ...
    results_disease.append(
        ['CFDNA+CFECC', args.DiseaseCase_samples_dir, blend_accuracy, blend_specificity, blend_recall, blend_f1, blend_auc]
    )
    roc.append(['CFDNA+CFECC', blend_mean_fpr1, blend_tpr_interp, blend_auc])
